...
parser.add_argument('--path_FE_model', type=str, default='models_EmoAffectnet/weights_0_66_49_wo_gl.h5',
                    help='Path to a model for feature extraction')
parser.add_argument('--path_LSTM_model', type=str, default='models_EmoAffectnet/TensorFlow/SAVEE_with_config.h5',
...
def pred_one_video(path):
    ...
    # Process face_areas in batches to avoid OOM
    batch_size = 32
    features = []
    for i in range(0, len(face_areas), batch_size):
        tf.keras.backend.clear_session()  # Clear session to free memory
        batch = face_areas[i:i + batch_size]
        batch = np.stack(batch)  # Stack into (batch_size, height, width, channels)
        batch = tf.convert_to_tensor(batch, dtype=tf.float32)  # GPU-compatible tensor
        batch_features = EE_model.predict(batch, verbose=0)  # Inference mode
