function dev_buttons()
    ...
    % A refresh button reloads scripts from the button_scripts folder.

    % Load button configurations from script files in button_scripts folder
...
function buttons = loadButtonsFromScripts()
    % Load button configurations from MATLAB script files in button_scripts folder
    buttons = {};
    
    % Define the script folder path (relative to the current script location)
    scriptFolder = fullfile(fileparts(mfilename('fullpath')), 'button_scripts');
    
    % Check if the folder exists
    if ~exist(scriptFolder, 'dir')
        % Try alternative path
        scriptFolder = 'support_functions/button_scripts';
        if ~exist(scriptFolder, 'dir')
            return;
        end
    end
    
    % Get all .m files in the button_scripts folder
